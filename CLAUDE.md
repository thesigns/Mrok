...
Mrok is a traditional roguelike game built as a Progressive Web App (PWA) using vanilla JavaScript and HTML5 Canvas. It targets mobile browsers in landscape orientation with offline capability, and also supports desktop play via keyboard and mouse.
...
- **`src/index.html`** — entry point: 800x360 scaled `<main>` container with two screens: `#menu-screen`, `#game-screen`. Game layout: `#panel-left` (minimap + compass) | 360x360 canvas | `#panel-right` (stats + button-grid). All 18 buttons have `data-key` attributes for keyboard hint overlays.
- **`src/js/main.js`** — creates `Game` instance, starts game immediately, scales layout via CSS transform (using `visualViewport` API with fallback), menu screen toggling, conditional PWA install button in menu via `beforeinstallprompt`
- **`src/js/game.js`** — `Game` class: owns canvas, `Session`, `Renderer`, `Minimap`, `Infobox`; handles compass input with auto-repeat; opens closed doors on bump; handle button for tile interactions with targeting mode (`getUsableTiles()`, `enterTargeting()`, `cancelTargeting()`, `useTile()`); calls FOV and renderer each frame; `Keyboard` for key input; mouse click-to-move via `Astar` pathfinding with overlay canvas cursor
...
- **`src/js/keyboard.js`** — `Keyboard` class: maps physical keys to callbacks (`onCompass`, `onHandle`, `onMenu`); own auto-repeat (500ms delay, 100ms interval); `isActive()` gate
- **`src/js/astar.js`** — `Astar` class: static `findPath(scene, sx, sy, tx, ty)` using A* with octile heuristic; 8-directional; treats closed doors as walkable; returns path array or null
...
- Each tile/critter is tinted via `source-atop` compositing on an offscreen canvas; tile colors defined in `tile.tint`, critter color in `critter.tint`
...
Menu button (`#btn-menu`, bottom-right of button-grid) toggles `#menu-screen` with title art, "Back to game", "Install app" (shown only when PWA install is available), and "Quit game".

Keyboard: Two-handed layout mirroring the screen panels. Left hand QWEASDZXC maps to compass (Q=NW, W=N, E=NE, A=W, S=wait, D=E, Z=SW, X=S, C=SE). Right hand TYUGHJBNM maps to button-grid (H=handle, M=menu, rest unused). Auto-repeat on compass keys matches touch timing. `Keyboard` class in `keyboard.js`, instantiated by `Game`. Keys ignored when game screen is hidden.

Mouse: Click on viewport canvas to move. Pixel mapped to tile via `floor(px/24)` + camera offset. Click on self = wait. Click elsewhere = A* pathfind, execute first step via `handleCompass()`. Hover shows `cursor.png` (24x24 corner-bracket selector) on overlay canvas. Desktop only (mousemove doesn't fire on touch).

Key hints: Each of the 18 buttons has a `data-key` attribute. CSS `::after` pseudo-element displays the key letter in bottom-right corner, visible only on desktop via `@media (hover: hover) and (pointer: fine)`.
...
- `src/gfx/gui/cursor.png` — 24x24 tile selection cursor (corner brackets) for mouse hover
